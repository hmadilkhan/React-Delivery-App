		...
		return (
			<React.Fragment>
				<div className="delivery-account-orders-block p-15 mb-20">
					<div className="d-flex justify-content-between mb-2">
						...
						<div>
							{localStorage.getItem("showFromNowDate") === "true" ? (
								<Moment fromNow>{order.order.date}</Moment>
							) : (
								<Moment format="DD/MM/YYYY hh:mma">{order.order.date}</Moment>
							)}
						</div>
					</div>
					...
					<div>
						<i className="si si-pointer mr-1" /> {order.order.customer.address}
					</div>
				</div>
			</React.Fragment>
		);
