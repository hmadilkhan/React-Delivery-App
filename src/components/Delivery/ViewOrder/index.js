...
class ViewOrder extends Component {
	...
	componentWillReceiveProps(nextProps) {
		if (nextProps.single_delivery_order.order.status === 2) {
		...
		if (nextProps.single_delivery_order.order.status === 11) {
		...
		if (nextProps.single_delivery_order.order.status === 10) {
		...
		if (nextProps.single_delivery_order.order.status === 4) {
			this.setState({ loading: false });
	...
	__acceptToDeliver = () => {
		this.props.acceptToDeliverOrder(
			...
			this.props.single_delivery_order.order.id
		);
		this.setState({ loading: true });
	};

	__pickedUp = () => {
		this.props.pickupOrder(this.props.delivery_user.data.auth_token, this.props.single_delivery_order.order.id);
		this.setState({ loading: true });
	};
	...
	getDeliveryGuyTotalEarning = (order) => {
		...
		return formatPrice(order.order.total_amount);
	};
									...
									<React.Fragment>
										...
										>
												...
												<ul className="list list-timeline list-timeline-modern delivery-address-timeline pull-t pb-20">
													<li>
														...
														<Flip bottom duration={500}>
															<div className="list-timeline-content">
																<p className="m-0 font-w700">{order.order.restaurant.branch_name}</p>
																<p className="m-0 single-order-restaurant-description">
																	{order.order.restaurant.description}
																</p>
																<p className="m-0">{order.order.restaurant.branch_address}</p>
																<p className="mb-2">{order.order.restaurant.code}</p>
																<div
																	onClick={() =>
													...
													<li className="mt-20">
														...
														<Flip bottom duration={1000}>
															<div className="list-timeline-content">
																<p className="font-w700">{order.order.customer.name}</p>
																<p className="mb-0">{order.order.customer.phone}</p>
																<p className="mb-2">{order.order.customer.address}</p>
																<div>
																	<button
																		className="btn btn-get-direction mr-2"
																		onClick={() =>
																			this.__getDirectionToUser(order.order.customer.address)
																		}
																	>
																		<i className="si si-action-redo mr-5" />
																		{localStorage.getItem(
																	...
																	<a
																		...
																		href={"tel:" + order.order.customer.mobile}
																	>
											...
											{this.state.picked_up && (
												<React.Fragment>
													<div className="pt-15 px-15">
														{order.order.payment_mode.payment_mode === "COD" ? (
